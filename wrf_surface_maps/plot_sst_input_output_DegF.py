...
def main(args):
    ...
    with open(configfile) as config:
        ...
        for k, v in config_info.items():
            if month in v['months']:
                ...
                color_lims_fahrenheit = [celsius_to_fahrenheit(temp) for temp in color_lims_celsius]
    ...
    for key, values in extents.items():
        ...
        save_file = os.path.join(values['save'], f'ru-wrf_{model}_sst_{ymd}.png')
        kwargs = dict()
        kwargs['zoom_coastline'] = False
        #kwargs['landcolor'] = 'none'
        if key == 'extent_bight_3km':
            save_file = os.path.join(values['save'], f'ru-wrf_{model}_sst_bight_{ymd}.png')
        ...
        pf.plot_pcolormesh_panel(fig, ax4, lon_wrf, lat_wrf, sst_wrf_sub_fahrenheit, **kwargs)
