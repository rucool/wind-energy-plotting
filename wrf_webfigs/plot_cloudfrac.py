...
def add_text_panel(ax, run_date, time_coverage_start, model):
    """
...
def plt_cloudfrac(nc, model, figname):
    ...
    for pt in plot_types:
        if pt == 'full_grid':
            ...
            max_cloudfrac_sub, ax_lims = subset_grid_panel(max_cloudfrac, model)
        else:
            # create a new file path
            ...
            max_cloudfrac_sub, ax_lims = subset_grid_panel(max_cloudfrac, 'bight')
        ...
        for i, level in enumerate(var['low_mid_high'].values):
            ...
            if pt == 'full_grid':
                # subset the entire grid
                ds_sub, ax_lims = subset_grid_panel(ds, model)
            else:
                # subset just NY Bight
                ds_sub, ax_lims = subset_grid_panel(ds, 'bight')
            ...
            if level == 300:
                ...
                add_text_panel(axes_ind[i], nc.SIMULATION_START_DATE, nc.time_coverage_start, model)
...
def subset_grid_panel(data, model):
    """
